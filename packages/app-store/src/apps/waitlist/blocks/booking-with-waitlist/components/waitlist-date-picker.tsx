...
import { groupWaitlistDates, WaitlistDateGroup } from "../../../models/utils";
...
export function WaitlistDatePicker({
  ...
  const formatDateRange = (startDate: string, endDate: string) => {
    const start = LuxonDateTime.fromISO(startDate).setLocale(locale);
    const end = LuxonDateTime.fromISO(endDate).setLocale(locale);

    if (startDate === endDate) {
      return start.toLocaleString(LuxonDateTime.DATE_HUGE);
    }

    return `${start.toLocaleString(LuxonDateTime.DATE_MED)} - ${end.toLocaleString(LuxonDateTime.DATE_MED)}`;
  };
